      ...
      {processedLogs.map((log, index) => {
        if (log.header === 'subquery_context_window' || log.header === 'differences') {
          ...
          return (
            <div
              key={index}
              className="w-full max-w-4xl mx-auto rounded-lg pt-2 mt-3 pb-2 px-4 bg-gray-50 border border-gray-200 shadow-sm"
            >
              <p className="py-3 text-base leading-relaxed text-gray-900">
                {log.text}
              </p>
            </div>
          );
